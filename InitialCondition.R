...
getInitialState=function(net,scenario,type){
  if(type=='random'){
...
for(type in Types){
  ...
  for (l in 1:nrow(Scenarios)) {
    ...
    data=runExperiment(net,Time,R,p_a,p_d,scenario,
                       algorithm = 'BKMC',num_points = Time,
                       type = type)
    data=cbind('Type'=type,'Scenario'=l,data)
    Data=rbind(Data,data)
    print(Sys.time()-t0)
  } 
}
Data$Type=factor(Data$Version,
                    labels=c('Fixed IC',
                             'Random IC'))
...
#########
setwd("Resultados")
if(!dir.exists('InitialCondition')){
  dir.create('InitialCondition')
}
setwd('InitialCondition')
save(Data,file='InitialCondition.RData')
...
foreach(i=1:length(net$genes),.packages = 'tidyverse')%dopar%{
  X=Data%>%
    select(Type,Scenario,Time,i+3)%>%
    pivot_longer(!c(Type,Scenario,Time))%>%
    summarise(`% Activation` = mean(value),
              .by = c(Type,Scenario,Time,name))%>%
    ggplot()+
